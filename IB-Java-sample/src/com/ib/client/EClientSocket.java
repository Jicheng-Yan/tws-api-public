...
public class EClientSocket {
	...
	// 54 = can receive scale order fields (PriceAdjustValue,
	...
	public synchronized void eConnect(String host, int port, int clientId, boolean extraAuth) {
		...
		try {
			...
			// jicheng eConnect(socket);
			eConnect(socket, m_clientId);
		} catch (Exception e) {
			eDisconnect();
			connectionError();
		}
	...
	public synchronized void reqScannerSubscription(int tickerId, ScannerSubscription subscription,
		...
		try {
			...
			if (m_serverVersion >= MIN_SERVER_VER_LINKING) {
				...
				int scannerSubscriptionOptionsCount = scannerSubscriptionOptions == null ? 0
						: scannerSubscriptionOptions.size();
	...
	public synchronized void reqHistoricalData(int tickerId, Contract contract, String endDateTime, String durationStr,
			String barSizeSetting, String whatToShow, int useRTH, int formatDate, List<TagValue> chartOptions) {
		// not connected?
		if (!m_connected) {
			notConnected();
			return;
	...
	public synchronized void reqRealTimeBars(int tickerId, Contract contract, int barSize, String whatToShow,
			boolean useRTH, Vector<TagValue> realTimeBarsOptions) {
		// not connected?
		if (!m_connected) {
			notConnected();
			return;
	...
	public synchronized void placeOrder(int id, Contract contract, Order order) {
		...
		if (m_serverVersion < MIN_SERVER_VER_DELTA_NEUTRAL_CONID) {
			if (order.m_deltaNeutralConId > 0 || !IsEmpty(order.m_deltaNeutralSettlingFirm)
					|| !IsEmpty(order.m_deltaNeutralClearingAccount) || !IsEmpty(order.m_deltaNeutralClearingIntent)) {
		...
		if (m_serverVersion < MIN_SERVER_VER_DELTA_NEUTRAL_OPEN_CLOSE) {
			if (!IsEmpty(order.m_deltaNeutralOpenClose) || order.m_deltaNeutralShortSale
					|| order.m_deltaNeutralShortSaleSlot > 0 || !IsEmpty(order.m_deltaNeutralDesignatedLocation)) {
				error(id, EClientErrors.UPDATE_TWS,
	...
	public synchronized void calculateImpliedVolatility(int reqId, Contract contract, double optionPrice,
			double underPrice) {
	...
	public synchronized void verifyRequest(String apiName, String apiVersion) {
		...
		if (m_serverVersion < MIN_SERVER_VER_LINKING) {
			error(EClientErrors.NO_VALID_ID, EClientErrors.UPDATE_TWS, "  It does not support verification request.");
			return;
		}
