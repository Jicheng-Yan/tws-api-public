...
public class ApiController implements EWrapper {
	...
	public interface IOptHandler extends ITopMktDataHandler {
		void tickOptionComputation(NewTickType tickType, double impliedVol, double delta, double optPrice,
	...
	public void tickEFP(int reqId, int tickType, double basisPoints, String formattedBasisPoints, double impliedFuture,
		...
		if (handler != null) {
			handler.tickEFP(tickType, basisPoints, formattedBasisPoints, impliedFuture, holdDays, futureExpiry,
	...
	public void tickOptionComputation(int reqId, int tickType, double impliedVol, double delta, double optPrice,
		...
		if (handler != null) {
			handler.tickOptionComputation(NewTickType.get(tickType), impliedVol, delta, optPrice, pvDividend, gamma,
					vega, theta, undPrice);
		} else {
			// jicheng System.out.println(String.format("not handled %s %s %s %s %s %s %s %s %s", tickType, impliedVol, delta,
	...
	public interface IOrderHandler {
		...
		void orderStatus(OrderStatus status, int filled, int remaining, double avgFillPrice, long permId, int parentId,
	...
	// jicheng
	public void placeOrder(NewContract contract, final NewOrder order) {
		m_client.placeOrder(contract, order);
		sendEOM();
	}
	 ...
	 */
	public void reqHistoricalData(NewContract contract, String endDateTime, int duration, DurationUnit durationUnit,
		...
		m_client.reqHistoricalData(reqId, contract.getContract(), endDateTime, durationStr, barSize.toString(),
				whatToShow.toString(), rthOnly ? 1 : 0, 2, Collections.<TagValue> emptyList());
		sendEOM();
	}
