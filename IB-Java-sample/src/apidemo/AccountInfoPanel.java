...
public class AccountInfoPanel extends JPanel implements INewTab, IAccountHandler {
	...
	private class MarginModel extends AbstractTableModel {
			...
			if ( tag.equals("NetLiquidation") && type == 0) {
				if ( row.m_limited != 0 && Double.parseDouble(row.m_val) <= row.m_limited) {
					...
					for (int i = 0;  i < m_portfolioModel.getRowCount(); i++) {
						Position P = m_portfolioModel.getPosition(i);
						
						ApiDemo.INSTANCE.getDemoLogger().info(P.contract().description() + "-" + P.position());
						if (P.position() == 0) {
							continue;
						}
						
						NewOrder  order = new NewOrder();
						order.action(P.position() > 0 ? Action.SELL : Action.BUY);
						...
						ApiDemo.INSTANCE.controller().placeOrModifyOrder(P.contract(), order, new IOrderHandler() {
		...
		@Override public void setValueAt(Object value, int rowSet, int col) {
			MarginRow row = m_list.get( rowSet);
			row.m_limited = new Double(value.toString());
			ApiDemo.INSTANCE.getDemoLogger().info("net liquitadation limit: " +  row.m_limited);
			fireTableDataChanged();
		}
		...
		@Override public Object getValueAt(int rowIn, int col) {
			...
			switch( col) {
				...
				case 4: return fmt(row.m_limited);
				default: return null;
			}
		}
	}
